package com.hufs.algoing.recommendation.algorithm;

import com.hufs.algoing.recommendation.dto.DailyRecommendDTO;
import com.hufs.algoing.problem.entity.Problem;
import com.hufs.algoing.problem.entity.UserSolvedProblem;
import com.hufs.algoing.user.entity.User;

import java.util.List;
import java.util.stream.Collectors;

public class DailyRecommenderAlgorithm {

    // 가중치 관리
    private static final double W_UserTier = 0.3;
    private static final double W_ProblemTier = 0.3;
    private static final double W_ProblemType = 0.2;
    private static final double W_UserSolvedType = 0.2;

    private static final int MAX_TIER_DIFF = 1; // 티어 차이 최대 +1까지

    // 유저 정보(user테이블) + 유저가 푼 문제(user solved problem테이블) + 문제(problem테이블) 리스트 받아서 추천리스트 만듦.
    public static List<DailyRecommendDTO> recommend(User user, List<Problem> problems, List<UserSolvedProblem> solvedProblems) {

        // 유저가 푼 문제 번호 -> USER_SOLVED_PROBLEM 테이블 Problem_num값 가져오기
        List<Long> solvedProblemIds = solvedProblems.stream()
                .filter(solvedProblem -> {
//                    System.out.println("Checking solvedProblem with userId: " + solvedProblem.getUserId().getUserId());
                    boolean isMatchingUser = solvedProblem.getUserId().getUserId().equals(user.getUserId());
//                    System.out.println("Does userId match? " + isMatchingUser);
                    return isMatchingUser;
                }) // 해당 유저가 푼 문제만 필터링
                .map(solvedProblem -> {
                    Long problemId = solvedProblem.getProblemId().getProblemId();
//                    System.out.println("Mapping problem ID: " + problemId);
                    return problemId; // 푼 문제의 문제번호만 추출
                })
                .collect(Collectors.toList());  // 리스트로 변환

//        System.out.println("Solved problem IDs: " + solvedProblemIds);


        // 유저가 푼 문제 유형(tag)
        List<String> userProblemTags = calculateUserProblemTags(user, solvedProblems, problems);
//        System.out.println("User problem tags: " + userProblemTags);

        // 문제 추천 점수 계산
//        System.out.println("Calculating problem recommendations...");
        List<DailyRecommendDTO> recommendations = problems.stream()
                .filter(problem -> !solvedProblemIds.contains(problem.getProblemId())) // 유저가 푼 문제 제외
                .map(problem -> {
                    // 각 문제 태그 추출
                    String problemTag = problem.getTag();
//                    System.out.println("Problem ID: " + problem.getProblemId() + ", Problem tag: " + problemTag);
                    double typeSimilarity = calculateCosineSimilarity(userProblemTags, problemTag); // 유사도 계산
//                    System.out.println("Cosine similarity for problem " + problem.getProblemId() + ": " + typeSimilarity);
                    // 점수 계산
                    double score = calculateScore(user, problem, userProblemTags, typeSimilarity);
                    System.out.println("Score for problem " + problem.getProblemId() + ": " + score);
                    // dailyRecommend 객체 생성(문제 번호와 점수)
                    return new DailyRecommendDTO(
                            problem.getProblemId(),
                            problem.getTitle(),        // 문제 제목
                            Math.toIntExact(problem.getLevel()),        // 문제 난이도
                            problem.getTag(),          // 문제 태그
                            score                      // 추천 점수
                    );                 // 추천 점수
                })
                .collect(Collectors.toList());

        // 점수 기준 정렬
        recommendations.sort((a, b) -> Double.compare(b.getScore(), a.getScore()));
//        System.out.println("Sorted recommendations: " + recommendations);

        // 상위 3개 문제만 추천
        int recommendCount = Math.min(3, recommendations.size());
//        System.out.println("Returning top " + recommendCount + " recommendations.");
        return recommendations.subList(0, recommendCount); // 상위 3개만 잘라서 반환
    }

    // 유저가 푼 문제 유형 리스트로 반환
    private static List<String> calculateUserProblemTags(User user, List<UserSolvedProblem> solvedProblems, List<Problem> problems) {
//        System.out.println("Calculating user problem tags for user: " + user.getUserId());
        List<String> userTags = solvedProblems.stream()
                .filter(solvedProblem -> solvedProblem.getUserId().equals(user)) // 유저가 푼 문제만 필터링
                .map(solvedProblem -> solvedProblem.getProblemId()) // 푼 문제 가져오기
                .map(problem -> problem.getTag()) // 태그 추출
                .distinct() // 중복 제거
                .collect(Collectors.toList());
//        System.out.println("User tags: " + userTags);
        return userTags;
    }

    // 유저가 푼 태그와 문제의 태그의 코사인 유사도 계산
    private static double calculateCosineSimilarity(List<String> userTags, String problemTag) {
        if (problemTag == null || problemTag.isEmpty()) {
//            System.out.println("Problem tag is null or empty. Returning 0.0 similarity.");
            return 0.0;
        }

        // 문제 태그를 ',' 기준으로 분리
        String[] problemTags = problemTag.split(",");
//        System.out.println("Problem tags after split: " + String.join(", ", problemTags));

        // 유저 태그와 문제 태그의 합집합을 만들어서, 각 태그가 포함되었는지 체크
        List<String> allTags = userTags.stream()
                .distinct() // 중복 제거
                .collect(Collectors.toList());
        for (String tag : problemTags) {
            if (!allTags.contains(tag)) {
                allTags.add(tag);
            }
        }
//        System.out.println("All unique tags (user + problem): " + String.join(", ", allTags));

        // 유저 태그와 문제 태그의 벡터 만들기
        int[] userVector = new int[allTags.size()];
        int[] problemVector = new int[allTags.size()];

        for (int i = 0; i < allTags.size(); i++) {
            if (userTags.contains(allTags.get(i))) {
                userVector[i] = 1;
            }
            if (List.of(problemTags).contains(allTags.get(i))) {
                problemVector[i] = 1;
            }
        }

        // 코사인 유사도 계산
        int dotProduct = 0;
        double userNorm = 0;
        double problemNorm = 0;

        for (int j = 0; j < allTags.size(); j++) {
            dotProduct += userVector[j] * problemVector[j];
            userNorm += userVector[j] * userVector[j];
            problemNorm += problemVector[j] * problemVector[j];
        }

        if (userNorm == 0 || problemNorm == 0) {
//            System.out.println("Either user or problem norm is 0. Returning 0.0 similarity.");
            return 0.0; // 벡터 크기가 0이면 유사도 0
        }

        double similarity = dotProduct / (Math.sqrt(userNorm) * Math.sqrt(problemNorm)); // 코사인 유사도 계산
//        System.out.println("Cosine similarity: " + similarity);
        return similarity;
    }

    private static double calculateScore(User user, Problem problem, List<String> userProblemTags, double typeSimilarity) {
        // 유저와 문제의 티어 차이
        double tierDiff = Math.abs(user.getTier() - problem.getLevel());
//        System.out.println("Tier difference between user and problem: " + tierDiff);

        // 유저와 문제의 티어 차이를 고려한 점수 부여
        double tierScore = (tierDiff <= MAX_TIER_DIFF) ? (2.0 - (tierDiff * 0.1)) : 0.0;
//        System.out.println("Tier score: " + tierScore);

        // 문제 유형 유사도 기반 점수
        double typeScore = typeSimilarity * W_ProblemType;
//        System.out.println("Type similarity score: " + typeScore);

        // 유저 문제 유형과 문제의 태그 유사도를 기반으로 점수 계산
        double solvedTypeScore = typeSimilarity * W_UserSolvedType;
//        System.out.println("Solved type score: " + solvedTypeScore);

        // 최종 점수 계산
        double totalScore = (W_UserTier * user.getTier() +
                W_ProblemTier * problem.getLevel() +
                typeScore +
                solvedTypeScore +
                tierScore);
//        System.out.println("Total score for problem " + problem.getProblemId() + ": " + totalScore);
        return totalScore;
    }
}
